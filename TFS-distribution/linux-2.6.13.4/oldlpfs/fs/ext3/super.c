...
#include <linux/init.h>
extern ssize_t ext3_readAllocStats(struct file *file, char __user *buf,
			    size_t count, loff_t *pos);

extern ssize_t ext3_getFreeCount(struct class_device * class, char*buf);
extern ssize_t ext3_getAllocCount(struct class_device * class, char*buf);
static CLASS_DEVICE_ATTR(Allocations, S_IRUGO,
		ext3_getAllocCount,NULL);
static CLASS_DEVICE_ATTR(Frees, S_IRUGO,
		ext3_getFreeCount,NULL);

struct file_operations ext3_stats_fops = {
	.owner	= THIS_MODULE,
	.open	= NULL,
	.read	= ext3_readAllocStats,
};
struct miscdevice ext3_stats_device = {
	.minor	= MISC_DYNAMIC_MINOR,
	.name	= "ext3stats",
	.fops	= &ext3_stats_fops,
};
...
static int __init init_ext3_fs(void)
{
	...
	if (err)
		goto out;
	/* JC - register ext3_stats_device */
	ret = misc_register(&ext3_stats_device);
	if(!ret){
		class = ext3_stats_device.class;
		class_device_create_file(class, &class_device_attr_Allocations);
		class_device_create_file(class, &class_device_attr_Frees);
	}
	/* /JC */
	return 0;
out:
