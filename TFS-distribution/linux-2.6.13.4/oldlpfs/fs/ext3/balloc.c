...
unsigned int ext3_block_allocs = 0;
unsigned int ext3_block_frees = 0;
			...
			unsigned long block, unsigned long count)
{
	...
	// JC - increment free count when blocks are freed
	// there is a bit of a race here, but as these are
	// just statistics, we can ignore it.
	ext3_block_frees+=dquot_freed_blocks;
...
allocated:
	...
	*errp = 0;
	...
	//JC - increment allocation count
	recordAlloc(ret_block);
...
/* JC - create a /sys interface for monitoring ext3 block
...
ssize_t ext3_getAllocCount(struct class_device *class, char *buf){
	unsigned int r;
	r = ext3_block_allocs;
	ext3_block_allocs = 0;
...
ssize_t ext3_getFreeCount(struct class_device *class, char *buf){
	unsigned int r;
	r = ext3_block_frees;
	ext3_block_frees = 0;
	return sprintf(buf,"%u\n",r);
...
#define ALLOC_STATS_BUFFER_SIZE 4000

unsigned int allocations[ALLOC_STATS_BUFFER_SIZE];
int alloc_write_cursor=0;
int alloc_read_cursor=0;

void recordAlloc(unsigned int block){
	ext3_block_allocs++;
	allocations[alloc_write_cursor] = block;
	alloc_write_cursor++;
...
void recordFree(unsigned int block){
	ext3_block_frees++;
}

ssize_t ext3_readAllocStats(struct file *file, char __user *buf,
			    size_t count, loff_t *pos)
{
	...
	for(i=0;i<count;i++){
		...
		if(copy_to_user(buffer+i, &v, sizeof(unsigned int))){
			i--;
			break;
		}
	}
