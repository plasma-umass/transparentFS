...
inline int lpfs_p_set_bit(int j, char *highp, char *lowp, char *dirty, char *blocklock, int priority){
	if(priority==0){
		if(lpfs_set_bit(j,blocklock)){
			return 1;
		}
		if(lpfs_set_bit(j,lowp)){
			lpfs_clear_bit(j,blocklock);
			return 1;
		}
		return 0;
		...
		if(lpfs_test_bit(j,lowp)){
			if(priority==-1){ printk("low priority metadata overwriting lp block\n"); }
			lpfs_set_bit(j,dirty);
		}else{
			if(lpfs_test_bit(j,dirty)){
				BUG();
			}
		}
...
int lpfs_p_set_bit_atomic(spinlock_t *lock, int j, char *highp, char *lowp, char *dirty, char *blocklock, int priority){
	int r;
	spin_lock(lock);
	r=  lpfs_p_set_bit(j, highp, lowp, dirty, blocklock, priority);
	spin_unlock(lock);
	return r;
}
