 ...
 */
static unsigned
lpfs_last_byte(struct inode *inode, unsigned long page_nr)
{
...
static void lpfs_check_page(struct page *page)
{
	...
	u32 max_inumber = le32_to_cpu(LPFS_SB(sb)->s_es->s_inodes_count);
...
lpfs_readdir (struct file * filp, void * dirent, filldir_t filldir)
{
	...
	for ( ; n < npages; n++, offset = 0) {
		...
		limit = kaddr + lpfs_last_byte(inode, n) - LPFS_DIR_REC_LEN(1);
			...
			struct dentry *dentry, struct page ** res_page)
{
	...
	struct lpfs_inode_info *ei = LPFS_I(dir);
	lpfs_dirent * de;
	...
	*res_page = NULL;
	...
	do {
		...
		if (!IS_ERR(page)) {
			...
			kaddr += lpfs_last_byte(dir, n) - reclen;
			while ((char *) de <= kaddr) {
				if (de->rec_len == 0) {
					lpfs_error(dir->i_sb, __FUNCTION__,
						"zero-length directory entry");
					lpfs_put_page(page);
					goto out;
				}
			...
			struct page *page, struct inode *inode)
{
	...
	LPFS_I(dir)->i_flags &= ~LPFS_BTREE_FL;
...
int lpfs_add_link (struct dentry *dentry, struct inode *inode)
{
	...
	for (n = 0; n <= npages; n++) {
		...
		lock_page(page);
		kaddr = page_address(page);
		dir_end = kaddr + lpfs_last_byte(dir, n);
		de = (lpfs_dirent *)kaddr;
...
int lpfs_delete_entry (struct lpfs_dir_entry_2 * dir, struct page * page )
{
	...
	while ((char*)de < (char*)dir) {
		if (de->rec_len == 0) {
			lpfs_error(inode->i_sb, __FUNCTION__,
				"zero-length directory entry");
			err = -EIO;
			goto out;
		}
	...
	LPFS_I(inode)->i_flags &= ~LPFS_BTREE_FL;
...
int lpfs_make_empty(struct inode *inode, struct inode *parent)
{
	...
	de = (struct lpfs_dir_entry_2 *)kaddr;
	de->name_len = 1;
	...
	de = (struct lpfs_dir_entry_2 *)(kaddr + LPFS_DIR_REC_LEN(1));
	de->name_len = 2;
...
int lpfs_empty_dir (struct inode * inode)
{
	...
	for (i = 0; i < npages; i++) {
		...
		kaddr = page_address(page);
		de = (lpfs_dirent *)kaddr;
		kaddr += lpfs_last_byte(inode, i) - LPFS_DIR_REC_LEN(1);
