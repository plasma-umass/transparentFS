...
struct dentry *lpfs_get_parent(struct dentry *child)
{
	...
	ino = lpfs_inode_by_name(child->d_inode, &dotdot);
	if (!ino)
	...
	const char * symname)
{
	...
	if (l > sizeof (LPFS_I(inode)->i_data)) {
		/* slow symlink */
		...
		memcpy((char*)(LPFS_I(inode)->i_data),symname,l);
		inode->i_size = l-1;
	}
	mark_inode_dirty(inode);
	...
	struct inode * new_dir,	struct dentry * new_dentry )
{
	 ...
	 * Like most other Unix systems, set the ctime for inodes on a
 	 * rename.
	 * lpfs_dec_count() will mark the inode dirty.
	 */
...
struct inode_operations lpfs_special_inode_operations = {
#ifdef CONFIG_LPFS_FS_XATTR
	.setxattr	= generic_setxattr,
	.getxattr	= generic_getxattr,
	.listxattr	= lpfs_listxattr,
