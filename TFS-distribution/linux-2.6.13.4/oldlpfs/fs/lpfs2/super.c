...
static int lpfs_remount (struct super_block * sb, int * flags, char * data);
static int lpfs_statfs (struct super_block * sb, struct kstatfs * buf);
		 ...
		 const char * fmt, ...)
{
	...
	if (!(sb->s_flags & MS_RDONLY)) {
		sbi->s_mount_state |= LPFS_ERROR_FS;
		es->s_state =
	...
	va_start(args, fmt);
	printk(KERN_CRIT "LPFS-fs error (device %s): %s: ",sb->s_id, function);
	vprintk(fmt, args);
	printk("\n");
	va_end(args);

	if (test_opt(sb, ERRORS_PANIC))
		panic("LPFS-fs panic from previous error\n");
		   ...
		   const char * fmt, ...)
{
	...
	va_start(args, fmt);
	printk(KERN_WARNING "LPFS-fs warning (device %s): %s: ",
	       sb->s_id, function);
	vprintk(fmt, args);
	printk("\n");
	va_end(args);
...
{
	struct lpfs_super_block *es = LPFS_SB(sb)->s_es;
...
static void lpfs_put_super (struct super_block * sb)
{
	int db_count;
	int i;
	struct lpfs_sb_info *sbi = LPFS_SB(sb);

	lpfs_xattr_put_super(sb);
	if (!(sb->s_flags & MS_RDONLY)) {
		struct lpfs_super_block *es = sbi->s_es;
	...
	kfree(sbi->s_debts);
        for(i=0;i<sbi->s_groups_count;i++){
                kfree(sbi->s_blocklock_bitmaps[i]);
        }
	kfree(sbi->s_blocklock_bitmaps);
...
static kmem_cache_t * lpfs_inode_cachep;

static struct inode *lpfs_alloc_inode(struct super_block *sb)
{
	struct lpfs_inode_info *ei;
	ei = (struct lpfs_inode_info *)kmem_cache_alloc(lpfs_inode_cachep, SLAB_KERNEL);
	if (!ei)
		return NULL;
...
static void lpfs_destroy_inode(struct inode *inode)
{
	kmem_cache_free(lpfs_inode_cachep, LPFS_I(inode));
}

static void init_once(void * foo, kmem_cache_t * cachep, unsigned long flags)
{
	struct lpfs_inode_info *ei = (struct lpfs_inode_info *) foo;
...
{
	lpfs_inode_cachep = kmem_cache_create("lpfs_inode_cache",
					     sizeof(struct lpfs_inode_info),
					     0, SLAB_RECLAIM_ACCOUNT,
					     init_once, NULL);
	if (lpfs_inode_cachep == NULL)
		return -ENOMEM;
	return 0;
}
...
{
	if (kmem_cache_destroy(lpfs_inode_cachep))
		printk(KERN_INFO "lpfs_inode_cache: not all structures were freed\n");
}

static void lpfs_clear_inode(struct inode *inode)
{
#ifdef CONFIG_LPFS_FS_POSIX_ACL
	struct lpfs_inode_info *ei = LPFS_I(inode);

	if (ei->i_acl && ei->i_acl != LPFS_ACL_NOT_CACHED) {
		posix_acl_release(ei->i_acl);
		ei->i_acl = LPFS_ACL_NOT_CACHED;
	}
	if (ei->i_default_acl && ei->i_default_acl != LPFS_ACL_NOT_CACHED) {
...
#ifdef CONFIG_QUOTA
static ssize_t lpfs_quota_read(struct super_block *sb, int type, char *data, size_t len, loff_t off);
static ssize_t lpfs_quota_write(struct super_block *sb, int type, const char *data, size_t len, loff_t off);
#endif
...
static struct super_operations lpfs_sops = {
	...
	.clear_inode	= lpfs_clear_inode,
#ifdef CONFIG_QUOTA
	.quota_read	= lpfs_quota_read,
	.quota_write	= lpfs_quota_write,
#endif
};

/* Yes, most of these are left as NULL!!
 * A NULL value implies the default, which works with lpfs-like file
 * systems, but can be improved upon.
 * Currently only get_parent is required.
 */
struct dentry *lpfs_get_parent(struct dentry *child);
static struct export_operations lpfs_export_ops = {
	.get_parent = lpfs_get_parent,
};
...
static unsigned long get_sb_block(void **data)
{
	...
	if (*options && *options != ',') {
		printk("LPFS-fs: Invalid sb specification: %s\n",
		       (char *) *data);
		return 1;
	}
	if (*options == ',')
			      ...
			      int read_only)
{
	...
	if (read_only)
		return res;
	if (!(sbi->s_mount_state & LPFS_VALID_FS))
		printk ("LPFS-fs warning: mounting unchecked fs, "
			"running e2fsck is recommended\n");
	else if ((sbi->s_mount_state & LPFS_ERROR_FS))
	...
	if (!le16_to_cpu(es->s_max_mnt_count))
		es->s_max_mnt_count = cpu_to_le16(LPFS_DFL_MAX_MNT_COUNT);
...
static int lpfs_check_descriptors (struct super_block * sb)
{
	...
	for (i = 0; i < sbi->s_groups_count; i++)
	{
		if ((i % LPFS_DESC_PER_BLOCK(sb)) == 0)
			gdp = (struct lpfs_group_desc *) sbi->s_group_desc[desc_block++]->b_data;
...
static int lpfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (!blocksize) {
		printk ("LPFS-fs: unable to set blocksize\n");
		goto failed_sbi;
	}
	...
	if (def_mount_opts & LPFS_DEFM_DEBUG)
		set_opt(sbi->s_mount_opt, DEBUG);
	if (def_mount_opts & LPFS_DEFM_BSDGROUPS)
		set_opt(sbi->s_mount_opt, GRPID);
	if (def_mount_opts & LPFS_DEFM_UID16)
		set_opt(sbi->s_mount_opt, NO_UID32);
	if (def_mount_opts & LPFS_DEFM_XATTR_USER)
		set_opt(sbi->s_mount_opt, XATTR_USER);
	if (def_mount_opts & LPFS_DEFM_ACL)
		set_opt(sbi->s_mount_opt, POSIX_ACL);
	
	if (le16_to_cpu(sbi->s_es->s_errors) == LPFS_ERRORS_PANIC)
		set_opt(sbi->s_mount_opt, ERRORS_PANIC);
	else if (le16_to_cpu(sbi->s_es->s_errors) == LPFS_ERRORS_RO)
	...
	sb->s_flags = (sb->s_flags & ~MS_POSIXACL) |
		((LPFS_SB(sb)->s_mount_opt & LPFS_MOUNT_POSIX_ACL) ?
		 MS_POSIXACL : 0);
	 ...
	 */
	features = LPFS_HAS_INCOMPAT_FEATURE(sb, ~LPFS_FEATURE_INCOMPAT_SUPP);
	if (features) {
	...
	if (!(sb->s_flags & MS_RDONLY) &&
	    (features = LPFS_HAS_RO_COMPAT_FEATURE(sb, ~LPFS_FEATURE_RO_COMPAT_SUPP))){
	...
	if (sb->s_blocksize != blocksize) {
		...
		es = (struct lpfs_super_block *) (((char *)bh->b_data) + offset);
		sbi->s_es = es;
		if (es->s_magic != cpu_to_le16(LPFS_SUPER_MAGIC)) {
	...
	sbi->s_frag_size = LPFS_MIN_FRAG_SIZE <<
	...
	if (LPFS_INODE_SIZE(sb) == 0)
		goto cantfind_lpfs;
	sbi->s_inodes_per_block = sb->s_blocksize / LPFS_INODE_SIZE(sb);
	...
	if (LPFS_BLOCKS_PER_GROUP(sb) == 0)
		goto cantfind_lpfs;
	sbi->s_groups_count = (le32_to_cpu(es->s_blocks_count) -
				        le32_to_cpu(es->s_first_data_block) +
				       LPFS_BLOCKS_PER_GROUP(sb) - 1) /
				       LPFS_BLOCKS_PER_GROUP(sb);
	db_count = (sbi->s_groups_count + LPFS_DESC_PER_BLOCK(sb) - 1) /
	...
	//FIXME - blocklock bitmaps need to be freed at some point too
        sbi->s_blocklock_bitmaps=kmalloc(sizeof(char *)*sbi->s_groups_count, GFP_KERNEL);
        if(sbi->s_blocklock_bitmaps==NULL){
                goto failed_mount2;
        }
        for(i=0;i<sbi->s_groups_count;i++){
                int j;
                sbi->s_blocklock_bitmaps[i]=kmalloc((LPFS_BLOCKS_PER_GROUP(sb)+7)/8, GFP_KERNEL);
                if(sbi->s_blocklock_bitmaps[i]==NULL){
                        goto failed_mount2;
                }
                for(j=0;j<(LPFS_BLOCKS_PER_GROUP(sb)+7)/8;j++){
                        sbi->s_blocklock_bitmaps[i][j]=0;
                }
        }
        /* /JC */ 
	 ...
	 */
	sb->s_op = &lpfs_sops;
	sb->s_export_op = &lpfs_export_ops;
	sb->s_xattr = lpfs_xattr_handlers;
	root = iget(sb, LPFS_ROOT_INO);
	...
	percpu_counter_mod(&sbi->s_freeblocks_counter,
				lpfs_count_free_blocks(sb));
	percpu_counter_mod(&sbi->s_freeinodes_counter,
				lpfs_count_free_inodes(sb));
...
cantfind_lpfs:
	if (!silent)
		printk("VFS: Can't find an lpfs filesystem on dev %s.\n",
		       sb->s_id);
	goto failed_mount;
...
{
	es->s_free_blocks_count = cpu_to_le32(lpfs_count_free_blocks(sb));
	es->s_free_inodes_count = cpu_to_le32(lpfs_count_free_inodes(sb));
	es->s_wtime = cpu_to_le32(get_seconds());
	mark_buffer_dirty(LPFS_SB(sb)->s_sbh);
	sync_dirty_buffer(LPFS_SB(sb)->s_sbh);
	sb->s_dirt = 0;
}
...
static int lpfs_remount (struct super_block * sb, int * flags, char * data)
{
	...
	sb->s_flags = (sb->s_flags & ~MS_POSIXACL) |
		((sbi->s_mount_opt & LPFS_MOUNT_POSIX_ACL) ? MS_POSIXACL : 0);
	...
	if (*flags & MS_RDONLY) {
		if (le16_to_cpu(es->s_state) & LPFS_VALID_FS ||
		    !(sbi->s_mount_state & LPFS_VALID_FS))
			return 0;
		/*
		...
		__le32 ret = LPFS_HAS_RO_COMPAT_FEATURE(sb,
...
{
	return get_sb_bdev(fs_type, flags, dev_name, data, lpfs_fill_super);
}
...
static ssize_t lpfs_quota_read(struct super_block *sb, int type, char *data,
			       size_t len, loff_t off)
{
	struct inode *inode = sb_dqopt(sb)->files[type];
	sector_t blk = off >> LPFS_BLOCK_SIZE_BITS(sb);
	int err = 0;
	...
	while (toread > 0) {
		...
		tmp_bh.b_state = 0;
		err = lpfs_get_block(inode, blk, &tmp_bh, 0);
		if (err)
			return err;
...
/* Write to quotafile */
static ssize_t lpfs_quota_write(struct super_block *sb, int type,
				const char *data, size_t len, loff_t off)
{
	...
	while (towrite > 0) {
		...
		tmp_bh.b_state = 0;
		err = lpfs_get_block(inode, blk, &tmp_bh, 1);
		if (err)
			goto out;
		if (offset || tocopy != LPFS_BLOCK_SIZE(sb))
			bh = sb_bread(sb, tmp_bh.b_blocknr);
		else
...
{
	int err = init_lpfs_xattr();
	if (err)
		return err;
	err = init_inodecache();
	if (err)
		goto out1;
        err = register_filesystem(&lpfs_fs_type);
	if (err)
		goto out;
	return 0;
out:
