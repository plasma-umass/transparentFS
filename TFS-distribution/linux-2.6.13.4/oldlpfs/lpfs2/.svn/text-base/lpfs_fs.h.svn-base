 ...
 */
/* JC - lpfs uses a new magic number */
...
struct lpfs_group_desc
{
	...
	__le16	bg_pad;
	/* JC - Pointers to bitmaps in group desc */
	__u32 bg_lowPriority_bitmap;
	__u32 bg_dirty_bitmap;
	__u32 bg_reserved[1];
	//__le32	bg_reserved[3]; ORIGINAL
	/* /JC */
};
...
/* JC - ioctl numbers for set ang get priority */
...
struct lpfs_inode {
	...
	/* JC - lp field in inode */
	/* FIXME
	I never changed the reserved space count
	so an lpfs inode is bigger than an ext2 inode.
	I am leaving it that way for compatability with
	older versions of lpfs. . . for now
	*/
	__u16	i_priority;
	/* /JC */
	union {
