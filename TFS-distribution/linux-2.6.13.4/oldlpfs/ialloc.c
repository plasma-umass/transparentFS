...
#include <linux/fs.h>
#include "lpfs_fs.h"
//#include <linux/locks.h>	//doesnt exist in 2.6?
					      ...
					      unsigned int block_group)
{
	...
	if (block_group >= sbi->s_groups_count)
		lpfs_panic (sb, "load_inode_bitmap",
	...
	if (sbi->s_groups_count <= LPFS_MAX_GROUP_LOADED) {
		slot = block_group;
	...
	else if (sbi->s_loaded_inode_bitmaps < LPFS_MAX_GROUP_LOADED)
...
void lpfs_free_inode (struct inode * inode)
{
	...
	if (!lpfs_clear_bit (bit, bh->b_data))
		lpfs_error (sb, "lpfs_free_inode",
...
{
	struct lpfs_super_block * es = LPFS_SBP(sb);
	int ngroups = LPFS_SBI(sb).s_groups_count;
...
/* JC timing functions for ialloc */

struct inode * lpfs_new_inode_helper (const struct inode * dir, int mode);

struct inode * lpfs_new_inode (const struct inode * dir, int mode){
	unsigned int j,i;
	struct inode *r;
#ifdef LPFS_BENCHMARK
	j=get_cycles();
#endif
	r=lpfs_new_inode_helper(dir, mode);
#ifdef LPFS_BENCHMARK
	i=get_cycles();
	if(i % LPFS_SAMPLE_FREQUENCY==0){
		...
		if(LPFS_IN(dir).i_priority==1){
			lpfs_hpialloc_time_total+=j;
			lpfs_hpialloc_time_count++;
		}else{
			lpfs_lpialloc_time_total+=j;
			lpfs_lpialloc_time_count++;
		}
	}
#endif
	return r;
...
repeat:
	if (S_ISDIR(mode))
		group = find_group_dir(sb, LPFS_IN(dir).i_block_group);
	else 
		group = find_group_other(sb, LPFS_IN(dir).i_block_group);
	...
	i = lpfs_find_first_zero_bit ((unsigned long *) bh->b_data,
				      LPFS_INODES_PER_GROUP(sb));
	if (i >= LPFS_INODES_PER_GROUP(sb))
		goto bad_count;
	lpfs_set_bit (i, bh->b_data);
	...
	ino = group * LPFS_INODES_PER_GROUP(sb) + i + 1;
	...
	LPFS_IN(inode).i_flags = LPFS_IN(dir).i_flags & ~LPFS_BTREE_FL;
	if (S_ISLNK(mode))
		LPFS_IN(inode).i_flags &= ~(LPFS_IMMUTABLE_FL|LPFS_APPEND_FL);
	LPFS_IN(inode).i_block_group = group;
	lpfs_set_inode_flags(inode);
	insert_inode_hash(inode);
	//inode->i_generation = event++;
	/* JC set new inode priority to priority of parent directory*/
	LPFS_IN(inode).i_priority=LPFS_IN(dir).i_priority;
	/* /JC */
...
fail2:
	desc = lpfs_get_group_desc (sb, group, &bh2);
	desc->bg_free_inodes_count =
		cpu_to_le16(le16_to_cpu(desc->bg_free_inodes_count) + 1);
	if (S_ISDIR(mode))
		desc->bg_used_dirs_count =
...
unsigned long lpfs_count_free_inodes (struct super_block * sb)
{
	...
	for (i = 0; i < LPFS_SBI(sb).s_groups_count; i++) {
		struct lpfs_group_desc *desc = lpfs_get_group_desc (sb, i, NULL);
		struct buffer_head *bh;
		unsigned x;
		...
		x = lpfs_count_free (bh, LPFS_INODES_PER_GROUP(sb) / 8);
	...
	return desc_count;
#else
	return le32_to_cpu(LPFS_SBI(sb).s_es->s_free_inodes_count);
#endif
}
