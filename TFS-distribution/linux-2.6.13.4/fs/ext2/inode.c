...
int ext2_step_blocks(struct inode *inode, int (*func)(struct inode *, int, int)){
	...
	for(i=0; i<blocks; i++){
		r=ext2_block_to_path(inode, i, offsets,NULL);
		...
		if(ext2_get_branch(inode, r, offsets, chain, &err)!=NULL){
			return -1;
		}
		for(j=1;j<r;j++){
			brelse((chain[j].bh));
