...
static int tfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	// JC - allocatate blocklock bitmaps
	// FIXME: these need to be freed at some point too
	...
	for(i=0; i < sbi->s_groups_count; i++){
		int j;
		sbi->s_blocklock_bitmaps[i] = kmalloc((TFS_BLOCKS_PER_GROUP(sb)+7)/8, GFP_KERNEL);
		if(sbi->s_blocklock_bitmaps[i]==NULL){
			//FIXME: free memory from successfull mallocs
			goto failed_mount2;
		}
		for(j=0; j< (TFS_BLOCKS_PER_GROUP(sb)+7)/8; j++){
			sbi->s_blocklock_bitmaps[i][j]=0;
		}
	}
	
	sbi->s_allocHint_bitmap = kmalloc(sizeof(char)*sbi->s_groups_count, GFP_KERNEL);
	if(sbi->s_allocHint_bitmap == NULL){
		//FIXME: free blocklock bitmaps
		goto failed_mount2;
	}
	for(i=0;i<sbi->s_groups_count;i++){
		sbi->s_allocHint_bitmap[i]=0;
	}
	sbi->s_blocks_per_allocHint = sbi->s_blocks_per_group/8;

	printk("\ndone allocating bitmaps\n");
...
/* JC - create a /sys interface for determining the number of allocations and frees */
#include <linux/device.h>
#include <linux/module.h>
#include <linux/miscdevice.h>
#include <linux/init.h>
extern ssize_t tfs_readAllocStats(struct file *file, char __user *buf,
			    size_t count, loff_t *pos);
extern ssize_t tfs_writeAllocHints(struct file *file, const char __user *buf,
				size_t count, loff_t *pos);
extern ssize_t tfs_getFreeCount(struct class_device * class, char*buf);
extern ssize_t tfs_getAllocCount(struct class_device * class, char*buf);


struct file_operations tfs_stats_fops = {
	.owner	= THIS_MODULE,
	.open	= NULL,
	.read	= tfs_readAllocStats,
	.write	= tfs_writeAllocHints,
};
struct miscdevice tfs_stats_device = {
