...
/*
* LPFS SIMPLY CLEARS THE LOW PRIORITY AND DIRTY BITMAPS
*/
	ext2fs_clear_block_bitmap(fs->lowPriority_block_map);
	ext2fs_clear_block_bitmap(fs->dirty_block_map);
/*
* END LPFS CODE
*/
...
/*
* LPFS LOW PRIORITY INODES ARE SIMPLY CLEARED
*/


	//	if( inode.lowPriority==0&&ext2fs_test_inode_bitmap(ctx->inode_used_map,ino)){
		if(inode.lowPriority==0 && LINUX_S_ISREG(inode.i_mode)){
	//	if(inode.lowPriority==0){
			lp=0;
	//		continue;
		}else{
			lp=1;
...
*/
		...
		if (ino == EXT2_BAD_INO) {
			...
			if(lp!=0){ext2fs_mark_inode_bitmap(ctx->inode_used_map, ino);}
...
static void process_inodes(e2fsck_t ctx, char *block_buf)
{
	...
	for (i=0; i < process_inode_count; i++) {
		...
		if(!(pctx.inode->lowPriority==0&&LINUX_S_ISREG(pctx.inode->i_mode))){
			check_blocks(ctx, &pctx, block_buf);
		}
		if (ctx->flags & E2F_FLAG_SIGNAL_MASK)
			break;
...
/*
		 * Mark block used for the low priority bitmap 
		 */
		if (fs->group_desc[i].bg_lowPriority_block_bitmap) {
			if (ext2fs_test_block_bitmap(ctx->block_found_map,
				     fs->group_desc[i].bg_lowPriority_block_bitmap)) {
				pctx.blk = fs->group_desc[i].bg_lowPriority_block_bitmap;
				if (fix_problem(ctx, PR_1_BB_CONFLICT, &pctx)) {
					ctx->invalid_block_bitmap_flag[i]++;
					ctx->invalid_bitmaps++;
				}
...
/*
		 * Mark block used for the dirty bitmap 
		 */
		if (fs->group_desc[i].bg_dirty_block_bitmap) {
			if (ext2fs_test_block_bitmap(ctx->block_found_map,
				     fs->group_desc[i].bg_dirty_block_bitmap)) {
				pctx.blk = fs->group_desc[i].bg_dirty_block_bitmap;
				if (fix_problem(ctx, PR_1_BB_CONFLICT, &pctx)) {
					ctx->invalid_block_bitmap_flag[i]++;
					ctx->invalid_bitmaps++;
				}
