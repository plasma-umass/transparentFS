...
errcode_t ext2fs_flush(ext2_filsys fs)
{
...
#ifdef EXT2FS_ENABLE_SWAPFS
	printf("ext2fs_flush: swapfs enabled\n");
	if (fs->flags & EXT2_FLAG_SWAP_BYTES) {
	...
	printf("ext2fs_flush: swapfs enabled done\n");
#else
	printf("ext2fs_flush: swapfs disabled\n");
	super_shadow = fs->super;
	group_shadow = fs->group_desc;
	printf("ext2fs_flush: swapfs disabled done\n");
#endif
	 ...
	 * (SUPERBLOCK_OFFSET).
	 */
	printf("ext2fs_flush: writing out master superblock\n");
	retval = write_primary_superblock(fs, super_shadow);
	printf("ext2fs_flush: done writing out master superblock\n");
	if (retval)
		goto errout;
	printf("ext2fs_flush: there was no problem with the master superblock\n");
	...
	printf("ext2fs_flush: not an external journal device\n");
	/*
	 ...
	 */
	printf("ext2fs_flush: writing out master group descriptors\n");
	...
	fs->super->s_block_group_nr = 0;
printf("ext2fs_flush: done writing out master group descriptors\n");
	 ...
	 */
printf("ext2fs_flush: writing out bitmaps\n");
	if (fs->write_bitmaps) {
		retval = fs->write_bitmaps(fs);
		if (retval)
			goto errout;
	}
printf("ext2fs_flush: done writing out bitmaps\n");
	fs->flags &= ~EXT2_FLAG_DIRTY;

	/*
	 * Flush the blocks out to disk
	 */
printf("ext2fs_flush: flushing blocks to disk\n");
	retval = io_channel_flush(fs->io);
printf("ext2fs_flush: done flushing blocks to disk\n");
errout:
